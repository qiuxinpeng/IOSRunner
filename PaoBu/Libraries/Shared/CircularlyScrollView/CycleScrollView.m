...
- (MyPageControl *)pageControl
{
    ...
    if (self.totalPageCount > 1) {
        if (!_pageControl) {
            ...
            CGFloat pageControlWidth = totalPageCounts * 5 + (totalPageCounts - 1) * dotGapWidth;
            CGRect pageControlFrame = CGRectMake(CGRectGetMidX(self.scrollView.frame) - 0.5 * pageControlWidth , 0.9 * CGRectGetHeight(self.scrollView.frame), pageControlWidth, 5);
...
- (void)setCurrentPageIndex:(NSInteger)currentPageIndex
{
    ...
    if (self.CurrentPageBlock) {
        self.CurrentPageBlock(_currentPageIndex);
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if(contentOffsetX >= (2 * CGRectGetWidth(scrollView.frame))) {
        ...
        //        QLog(@"next，当前页:%d",self.currentPageIndex);
        [self configContentViews];
    }
    if(contentOffsetX <= 0) {
        self.currentPageIndex = [self getValidNextPageIndexWithPageIndex:self.currentPageIndex - 1];
        //        QLog(@"previous，当前页:%d",self.currentPageIndex);
        [self configContentViews];
    }
}
...
- (void)longTapGestureAction:(UILongPressGestureRecognizer *)tapGesture
{
    if (tapGesture.state == UIGestureRecognizerStateBegan) {
        QLog(@"UIGestureRecognizerStateBegan");
        [self.animationTimer pauseTimer];
    }
    if (tapGesture.state == UIGestureRecognizerStateEnded) {
        [self.animationTimer resumeTimer];
        QLog(@"UIGestureRecognizerStateEnded");
    }
}
